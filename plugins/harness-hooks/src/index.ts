...
const spawnedSessionKeys = new Set<string>();

import yaml from "js-yaml";
const yamlParse = (str) => {
  try { return yaml.load(str) || { version: "0.0.0", gates: [] }; }
  catch { return { version: "0.0.0", gates: [] }; }
};
...
function withStateLock(fn) {
  ...
  try {
    ...
    try { rmdirSync(STATE_LOCK); } catch {}
  }
}
...
function callArbiter(stage, reportText, contractPath, ct) { // sync fallback
  try {
    ...
    if (contractPath) args.push(`--contract`, contractPath);
    if (ct) args.push(`--ct`, ct);
...
function intentParser(userPrompt, currentState) {
  ...
  for (const route of INTENT_ROUTES) {
    ...
    for (const pat of route.secondary) {
      ...
      if (m) {
        score += 0.2;
        if (!matchedKeyword) matchedKeyword = m[0];
        break;
      }
    }
  ...
  if (bestMatch && bestMatch.intent_type === "memory_query") {
    ...
    if (/记住|记录|save|store|写入/.test(lowerText)) sub_intent = "save";
    else if (/回忆|回想|还记得|recall|query|search|查.*记忆/.test(lowerText)) sub_intent = "retrieve";
    else sub_intent = "general";
  } else if (bestMatch && bestMatch.intent_type === "knowledge_query") {
    const lowerText = text.toLowerCase();
    if (/https?:|url|link|网站|article|page/.test(lowerText)) sub_intent = "web_fetch";
    else if (/搜索|search|google|baidu|查.*资料/.test(lowerText)) sub_intent = "web_search";
    else sub_intent = "concept";
  } else if (bestMatch && bestMatch.intent_type === "system_maintenance") {
    const lowerText = text.toLowerCase();
    if (/gateway/.test(lowerText)) sub_intent = "gateway";
    else if (/plugin/.test(lowerText)) sub_intent = "plugin";
    else if (/config|openclaw\.json|schema/.test(lowerText)) sub_intent = "config";
    else sub_intent = "general";
  }
...
function checkActionGate(intent, toolName, sessionId) {
  if (!intent || intent.intent_type === "unknown" || !intent.target) return null;
...
function isSkillWorkshopProposalCreation(event) {
  ...
  if (!toolName.includes("skill_workshop")) return false;
...
function evaluateSoulGates(text, state, options = {}) {
  ...
  for (const gate of gates) {
    if (skipGateIds.has(gate.id)) continue;
    if (!gate.triggers) continue;
    let triggered = false;
...
function classifyRiskLevel(event) {
  ...
  for (const p of config.patterns) {
    try {
      ...
      if (regex.test(text)) return { severity: p.severity || "HIGH", preflight: p.preflight !== false, warning: p.preflight_warning || "", reason: p.reason || "" };
...
function escalateModelTier(state, reason) {
  ...
  if (state.state === "SUSPECTED_STUCK") return false;
  const tiers = getModelTiers();
...
function advanceToNextCT(state) {
  ...
  if (!current || !state.ctSequence || state.ctSequence.length === 0) return false;
  if (!state.completedCTs) state.completedCTs = [];
...
function detectModelFailure(event) {
  ...
  if (!combined || combined.length < 20) return "empty response";
  for (const pat of getModelDeadPatterns()) {
    if (pat.test(combined)) return combined.substring(0, 80);
  }
  return null;
}
...
function getRecentAuditLog(state, count) {
  if (!state.auditLog || state.auditLog.length === 0) return [];
...
function shouldTriggerSpawn(state) {
  ...
  if (state.state !== "SPEC_REVIEW" && state.state !== "QUALITY_REVIEW") return false;
  const recent = getRecentAuditLog(state, 5);
  if (recent.length > 0) {
    // Check recent audit entries for coder handoff pattern
    const hasCoderActivity = recent.some(e =>
      e.type === "handoff" || (e.tool === "write" && e.target && e.target.includes("handoff"))
    );
    const hasNoReviewerSpawn = !recent.some(e =>
      e.tool === "sessions_spawn" && e.target && (e.target.includes("reviewer") || e.target.includes("review") || e.target.includes("spec") || e.target.includes("quality"))
    );
    if (hasCoderActivity && hasNoReviewerSpawn) return true;
    return false;
  }
...
function checkRestartPreflightGate(event) {
  ...
  if (cmd.includes("node --check")) return null;
...
function _mergeConcurrent(disk, base, ours) {
  ...
  if (_deepEqual(base, ours)) return disk;
  if (disk == null) return ours;
  if (Array.isArray(base) && Array.isArray(ours)) {
    if (!Array.isArray(disk)) return ours;
  ...
  if (_isPlainObj(base) && _isPlainObj(ours) && _isPlainObj(disk)) {
    ...
    for (const k of Object.keys(ours)) {
      if (STATE_META_KEYS.has(k)) continue;
      if (_deepEqual(base[k], ours[k])) continue; // unchanged by us → keep disk[k]
...
function rotateTransitionsLogIfNeeded() {
  try {
    if (!existsSync(TRANSITIONS_LOG)) return;
    ...
    if (stats.size > LOG_ROTATE_THRESHOLD_BYTES) {
      ...
      try { unlinkSync(TRANSITIONS_LOG + ".20"); } catch { /* ignore if not exists */ }
      // Shift backups: .19→.20, .18→.19, ..., .1→.2
      for (let i = MAX_BACKUPS - 1; i >= 1; i--) {
        const src = `${TRANSITIONS_LOG}.${i}`;
        const dst = `${TRANSITIONS_LOG}.${i + 1}`;
        try { renameSync(src, dst); } catch { /* ignore if source doesn't exist */ }
      }
...
function runContractVerify(state) {
  ...
  if (!contractPath) return { passed: true, summary: "no contract file found, skipping verify", results: [] };
  const cmds = extractVerifyCommands(contractPath);
  if (cmds.length === 0) return { passed: true, summary: "no verify commands in contract, skipping", results: [] };
  const results = [];
...
function findLatestLC() {
  if (!existsSync(CONTRACTS_DIR)) return null;
  let latest = null;
  let latestMtime = 0;
  try {
    const files = readdirSync(CONTRACTS_DIR);
    for (const f of files) {
      if (!f.startsWith("LC-")) continue;
      const ext = f.substring(f.lastIndexOf("."));
      if (!LC_EXTENSIONS.includes(ext.toLowerCase())) continue;
  ...
  try {
    ...
    if (cts.length === 0) {
      ...
      for (const m of tableMatches) {
        ...
        if (id && !seen.has(id[1])) { seen.add(id[1]); cts.push(id[1]); }
      }
    }
    ...
    if (cts.length === 0) {
      ...
      for (const m of anyMatches) {
        if (!seen.has(m)) { seen.add(m); cts.push(m); }
      }
    }
...
function checkLCActivation(state) {
  ...
  if (state.lastSeenLC === lc.name && state.state !== "INACTIVE") return;
...
function inferAgentTypeFromConfig(combined: string): string | null {
  const cfg = loadConfig();
...
function isResumeRetryCommand(text: string): boolean {
  return /继续|继续执行|resume|continue|retry|重试/.test(text);
}
function parseReviewVerdict(reportFile: string): { isAccept: boolean; isRevise: boolean; isBlocked: boolean } | null {
...
function buildLongTaskGrillInject(): string[] {
  return [
...
function isStaleSubagent(info: any, now: number, staleMs = 60000): boolean {
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        if (state.state !== "INACTIVE" && state.activeContract) {
          ...
          if (modelCtx.length > 0) injects.push(...modelCtx);
        ...
        if (agentType === "coder") {
          ...
          for (const dir of ["handoffs", "artifacts"]) {
            ...
            try {
              ...
              for (const f of entries as string[]) {
    ...
    api.on(
      ...
      async (event, ctx) => {
        ...
        if (event.toolName === "sessions_spawn") {
          ...
          let agentType = inferAgentTypeFromConfig(combined);
          if (!agentType && (params.taskName || "").includes("coder")) agentType = "coder";
          if (!agentType && (params.taskName || "").includes("review")) agentType = "reviewer";
        ...
        if (event.toolName === "write" || event.toolName === "edit") {
          ...
          if (crit && (p.endsWith(".js") || p.endsWith(".mjs"))) {
            ...
            execFile("node", ["--check", p], { timeout: 5000 }, (e) => {
              if (e) console.log("[syntax-check FAIL] " + p);
            });
          }
          if (crit && (p.endsWith(".json"))) {
            try { JSON.parse(readFileSync(p, "utf-8")); }
            catch (e) { console.log("[config-error] " + p); }
          }
        }

        if (state.state === "INACTIVE") { saveState(state); return; }
        ...
        if (isContractRuntimeActive(state) && ["CT_RUNNING", "SPEC_REVIEW", "QUALITY_REVIEW", "SUSPECTED_STUCK", "ARBITER_BLOCKED"].includes(state.state)) {
          ...
          if (!state.recentToolCalls) state.recentToolCalls = [];
          ...
          const toolCounts = {};
          last10.forEach(c => { if (!["exec", "read"].includes(c.tool)) toolCounts[c.tool] = (toolCounts[c.tool] || 0) + 1; });
