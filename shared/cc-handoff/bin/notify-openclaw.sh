...
TASK="${1:-}"
TARGET="${2:-${NOTIFY_TARGET:-guanj_oc}}"
...
# 旧格式兼容：cc.notify.flag 是 guanj_oc 旧守护（handoff-daemon.sh §6）的消费路径——
# 它只在 LOCAL_NODE=guanj_oc 时被读取/清理。因此过渡期 notify guanj_oc 时仍写一份，
# 保证未升级到对称守护的 oc 节点也能收到 DONE 通知；全节点升级后可删此分支。
# （定向给其它节点的通知不写 cc.notify.flag：它不是那些节点的消费路径，写了是噪声。）
if [ "$TARGET" = "guanj_oc" ]; then
